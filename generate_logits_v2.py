...
parser.add_argument(
    "--datapath", required=True, type=str, help="directory from where to load datasets"
)
parser.add_argument(
    "--modelpath",
    required=True,
...
parser.add_argument(
    "--save_dir", required=True, type=str, help="directory to save logits"
)
parser.add_argument(
    "--dataset",
    default="CIFAR10",
...
parser.add_argument(
    "--alpha", default=0.1, type=float, help="alpha for dirichlet distribution"
)
parser.add_argument(
    "--seed", default=90, type=int, help="seed for random number generators"
)
parser.add_argument(
    "--proxy_ratio", default=0.1, type=float, help="ratio of proxy set to training set"
)
parser.add_argument(
    "--batch_size", default=16, type=int, help="batch size for logit generation"
)
parser.add_argument(
    "--diff_init",
    action="store_true",
...
parser.add_argument(
    "--generate_valset", action="store_true", help="whether to generate valset logits"
)
parser.add_argument(
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.save_dir):
        if os.path.exists(args.save_dir + "/logit_trainset.pkl") and os.path.exists(
            args.save_dir + "/logit_testset.pkl"
        ):
            print("==> Logits already exist. Exiting...")
            exit(0)

    model = util.select_model(args)
    models, labels = util.load_models_and_label_dist(
        model, args.modelpath, map_location="cpu"
    )
    ...
    print(
        f"==> Time taken to generate trainset logits: {gap_min} min {gap_secs:.2f} secs"
    )
    torch.save(
        all_train_logits, args.save_dir + "/logit_trainset.pth", pickle_protocol=2
    )
    ...
    gap_secs = (end - start) % 60
    print(
        f"==> Time taken to generate testset logits: {gap_min} min {gap_secs:.2f} secs"
    )
    ...
    if args.generate_valset:
        ...
        gap_secs = (end - start) % 60
        print(
            f"==> Time taken to generate valset logits: {gap_min} min {gap_secs:.2f} secs"
        )
        torch.save(
            all_val_logits, args.save_dir + "/logit_valset.pth", pickle_protocol=2
        )
    ...
    if args.generate_local_testset:
        ...
        gap_secs = (end - start) % 60
        print(
            f"==> Time taken to generate local testset logits: {gap_min} min {gap_secs:.2f} secs"
        )
        torch.save(
            all_local_test_logits,
            args.save_dir + "/logit_local_testset.pth",
            pickle_protocol=2,
        )
