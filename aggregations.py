...
def get_competencies(classes, competency_matrix):
    ...
    competencies_to_send = [
        [competency_matrix[i][k] for i in range(num_classes)] for k in classes
    ]
...
def _check_accuracy(
    weights, bias, testset, total_clients, num_classes, metric=None, require_argmax=True
):
    ...
    with torch.no_grad():
        for X, y in testset:
            ...
            y_pred = (
                torch.argmax(y_pred_probits, dim=1)
                if require_argmax
                else y_pred_probits
            )
...
def _train_and_evaluate(
    ...
    while epoch < iterations:
        ...
        logging.debug(
            "Epoch {ep} Loss {loss.avg:.4f} Train Acc {top1.avg:.3f}".format(
        ...
        if epoch % test_every == 0:
            test_acc = _evaluate(
                model, test_loader, device, metric=metric, require_argmax=require_argmax
            )
...
def evaluate_all_aggregations(
    ...
    for X, y in testset:
        ...
        for elem_idx in range(X.size(0)):
            ...
            y_pred_for_elem_idx = get_prediction_using_competency(
    ...
    for e in range(epochs):
        ...
        if e % 20 == 0:
            ...
            linear_mapping_performance = _check_accuracy(
                weights,
                bias,
                testset,
                total_clients,
            ...
            logging.info(
                f"Epoch {e + 1} Linear Mapping Performance: {linear_mapping_performance}"
            )
            if linear_mapping_performance > best_acc:
                logging.debug(
                    f"Improved performance from {best_acc:.4f} to {linear_mapping_performance:.4f}"
                )
    ...
    best_acc, _ = _train_and_evaluate(
        f,
        lossfunc,
        optimizer,
        scheduler,
