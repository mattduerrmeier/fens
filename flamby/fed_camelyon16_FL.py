...
result_dir = "../results/flamby/fed_camelyon16"
...
for strat in tuning_strategies:
    for round in rounds:
        for seed in seeds:
            ...
            print(f"Running {strat.__name__} with {round} rounds for seed {seed}")

            args["seed"] = seed
            args["nrounds"] = round
            args["learning_rate"] = config["strategies"][strat.__name__][
                "learning_rate"
            ]
            args["model"] = Baseline()
            ...
            performance = evaluate_model_on_tests(
                trained_model, test_dataloaders, metric
            )
...
# Compute mean performance for each round
for strat in tuning_strategies:
    mean_perfs = []
    for round in rounds:
        perfs_for_round = [v for r, v in result_dict[strat.__name__] if r == round]
        mean_perf = sum(perfs_for_round) / len(perfs_for_round)
        mean_perfs.append(mean_perf)
        print(f"Mean performance for {strat.__name__} with {round} rounds: {mean_perf}")

    result_dict[strat.__name__] = list(zip(rounds, mean_perfs))
