...
def load_trainset_fedcam16(client_id, FedDataset, num_classes, proxy_frac=0.1, seed=90):
    ...
    for y, x in train_data.items():
        n_i = len(x) - int(proxy_frac * len(x))
        all_trainset.extend(
            [(a1, torch.tensor(y, dtype=torch.float32), a2) for a1, a2 in x[:n_i]]
        )
        all_proxyset.extend(
            [(a1, torch.tensor(y, dtype=torch.float32), a2) for a1, a2 in x[n_i:]]
        )
...
def generate_logits_combined(
    ...
    for i in range(num_clients):
        ...
        train_dataset, proxy_dataset, _ = load_trainset_combined(
            dataset_name, i, FedDataset, num_classes, proxy_frac, seed
        )
...
def generate_logits(
    ...
    for i in range(num_clients):
        ...
        train_dataset, proxy_dataset, _ = load_trainset(
            i, FedDataset, num_classes, proxy_frac=proxy_frac, seed=seed
        )
        train_dataloader = torch.utils.data.DataLoader(
...
def generate_logits_fedcam16(
    ...
    for i in range(num_clients):
        ...
        train_dataset, proxy_dataset, _ = load_trainset_fedcam16(
            i, FedDataset, num_classes, proxy_frac=proxy_frac, seed=seed
        )
        train_dataloader = torch.utils.data.DataLoader(
            train_dataset,
...
def process_batch_inferences(batch_inferences, weighting_matrix):
    ...
    for k in range(len(inferences_per_elem)):
        weighted_average = get_weighted_average(
            inferences_per_elem[k], weighting_matrix
        )
