...
if __name__ == "__main__":
    ...
    if args.dataset == "CIFAR10":
        ...
        testset = torchvision.datasets.CIFAR10(
            root=args.datapath, train=False, download=True, transform=transform_train
        )
    ...
    # with the extended training set
        ...
        trainset = torchvision.datasets.SVHN(
            root=args.datapath, split="train", download=True, transform=transform_train
        )
        extraset = torchvision.datasets.SVHN(
            root=args.datapath, split="extra", download=True, transform=transform_train
        )
        trainset = ConcatDataset([trainset, extraset])

        testset = torchvision.datasets.SVHN(
            root=args.datapath, split="test", download=True, transform=transform_train
        )
    ...
    train_loader = torch.utils.data.DataLoader(
        trainset, batch_size=512, shuffle=False, num_workers=4
    )
    ...
    torch.save(
        flattened_logit_trainset,
        args.save_dir + "/logit_trainset.pth",
        pickle_protocol=2,
    )
    torch.save(
        flattened_logit_testset, args.save_dir + "/logit_testset.pth", pickle_protocol=2
    )
