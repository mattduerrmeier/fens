...
def run(rank, size):
    ...
    require_argmax = (
        dataset.require_argmax if hasattr(dataset, "require_argmax") else True
    )
    ...
    if hasattr(dataset, "criterion"):
        ...
        criterion = (
            nn.CrossEntropyLoss().cuda() if (args.gpu) else nn.CrossEntropyLoss()
        )
    ...
    if args.use_scheduler and args.optimizer == "localSGD":
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=args.highE, eta_min=0.001
        )
    else:
        scheduler = None
    ...
    for rnd in range(args.rounds):
        if (
            args.optimizer == "scaffold" or args.rounds == 1
        ):  # local training or scaffold
            selected_client_indexes = np.arange(
                total_clients
            )  # scaffold must fix process --> client mapping
        else:
            selected_client_indexes = rng.choice(
                range(total_clients), size=args.numclients, replace=False
            )
        ...
        ratios, tau_i = get_ratios(
            local_epochs, num_samples_this_round, rank, args.max_itr
        )
        optimizer.set_ratio(ratios[rank])
        logging.debug(
            "Client id {} local iterations {} weight {:.4f}".format(
                client_index, tau_i, ratios[rank]
            )
        )
        ...
        if rnd % args.evalafter == 0 or rnd == args.rounds - 1:
            ...
            if rank == 0 or args.rounds == 1:
                eval_model = (
                    best_model if args.rounds == 1 and best_model else model
                )  # use best model when local training
                ...
                # record metrics
                logging.info(
                    "Round {} test accuracy {:.3f} time {:.3f}".format(
                        rnd, test_acc, comm_time
                    )
                )
                with open(args.out_fname_test, "+a") as f:
                    print(
                        "{ep},{val:.4f},{time:.4f}".format(
                            ep=rnd, val=test_acc, time=comm_time
                        ),
                        file=f,
                    )
                ...
                if val_loader and args.rounds > 1:
                    val_acc = evaluate(
                        model, val_loader, metric=metric, require_argmax=require_argmax
                    )
                    with open(args.out_fname_global_val, "+a") as f:
                        print(
                            "{ep},{test:.4f},{time:.4f}".format(
                                ep=rnd, test=val_acc, time=comm_time
                            ),
                            file=f,
                        )
                    wandb.log(
        ...
        logging.info(
            "Rnd {} Worker {} Client id {} best test accuracy {:.3f}".format(
                rnd, rank, client_index, best_test_accuracy
            )
        )

        if rank == 0 and args.stop_early:
            if check_stopping_criteria(
                args.optimizer, args.dataset, args.alpha, best_test_accuracy, rnd
            ):
    ...
    if args.save_model and args.rounds == 1 and args.optimizer == "localSGD":
        ...
        if best_model:
            ...
            logging.info(
                "Saved best model {} to {}".format(
                    rank, f"{folder_name}/{rank}_best_model.pth"
                )
            )

        # save the label distribution as a list
        assert client_index == rank, (
            "Client index and rank should be the same for local training"
        )
    ...
    if (
        ...
        and args.rounds == 1
        and rank == 0
        and args.optimizer == "localSGD"
        and args.proxy_set
    ):
        all_proxy_loader = dataset.get_proxyset()
        models, label_dists = util.load_models_and_label_dist(model, folder_name)
        result_dict = evaluate_all_aggregations(
            all_proxy_loader, test_loader, models, label_dists, args
        )

        best_models, best_label_dists = util.load_models_and_label_dist(
            model, folder_name, use_best=True
        )
        result_dict_best = evaluate_all_aggregations(
            all_proxy_loader, test_loader, best_models, best_label_dists, args
        )
...
def train(
    ...
    while count < iterations:
            ...
            if batch_idx % args.print_freq == 0 and args.save:
                ...
                with open(args.out_fname, "+a") as f:
                    print(
                        ...
                        "{top1.val:.3f},{top1.avg:.3f},{cid}".format(
                            ep=epoch,
                            itr=batch_idx,
                            loss=losses,
                            top1=top1,
        ...
        if local_test_loader:  # name of variable to be changed
            ...
            val_acc = evaluate(
                model, local_test_loader, metric=metric, require_argmax=require_argmax
            )
            ...
            logging.info(
                "epoch {} client id {} local validation accuracy {:.3f}".format(
                    epoch, client_id, val_acc
                )
            )

            if not args.isEpochs:
                wandb.log({"val_acc": val_acc, "val_step": args.global_itr})
    ...
    with open(args.out_fname, "+a") as f:
        print(
            ...
            "{loss.val:.4f},{loss.avg:.4f},"
            "{top1.val:.3f},{top1.avg:.3f},{cid}".format(
                ep=epoch, itr=batch_idx, loss=losses, top1=top1, cid=client_id
            ),
            file=f,
        )
...
def get_ratios(local_epochs, num_samples_this_round, rank, tau_max=0):
    ...
    if args.isEpochs:
        for i, epochs_i in enumerate(local_epochs):
            ...
            tau_required_i = max(
                floor(epochs_i * tau_possible_i), 1
            )  # has to be atleast 1
            ...
            if tau_possible_i <= tau_required_i:  # more than or equal to one epoch
                ...
                num_seen_samples.append(
                    args.bs * max(tau_required_i, 1)
                )  # should see at least one batch

            if i == rank:
                tau_i = tau_required_i
    else:
...
def init_processes(rank, size, fn):
    ...
    dist.init_process_group(
        ...
        rank=rank,
        world_size=size,
        timeout=datetime.timedelta(seconds=3600 * 8),
    )  # 8 hours
    fn(rank, size)
...
if __name__ == "__main__":
    ...
    if args.gpu:  # gpu
        if not torch.cuda.is_available():
            raise ValueError(
                f"args.gpu set to {args.gpu}. GPU not available/detected on this machine."
            )
        else:
