...
class FedNova(Optimizer):
    ...
    def __init__(
        ...
        if mu != 0:
            raise ValueError(
                "Current version of the algorithm should not be used with proximal updates"
            )
        if dampening != 0:
            raise ValueError("Current version of the algorithm uses no dampening")
        if nesterov:
            raise ValueError(
                "Current version of the algorithm should not be used with Nesterov updates"
            )
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if "cum_grad" not in param_state:
                    param_state["cum_grad"] = torch.tensor(d_p)
