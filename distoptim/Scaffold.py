...
class Scaffold(Optimizer):
    ...
    def __init__(
        ...
        if self.clients_per_round <= 0:
            raise ValueError(
                "Invalid clients per round: {}".format(self.clients_per_round)
            )
        if self.total_clients <= 0:
            raise ValueError("Invalid total clients: {}".format(self.total_clients))
        if self.clients_per_round != self.total_clients:
            raise ValueError(
                "Current implementation only supports full client participation"
            )
            ...
            for p in group["params"]:
                ...
                with torch.no_grad():
                    ...
                    c_i_plus = (
                        param_state["c_i"]
                        - param_state["c"]
                        + (param_state["old_init"] - p.data) / (self.itr * group["lr"])
                    )
                    param_state["c"].mul_(1 / self.clients_per_round)
                    param_state["c"].add_(
                        1 / self.total_clients, c_i_plus - param_state["c_i"]
                    )
