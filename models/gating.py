...
class FederatedMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        gating_coefficients_normalised = F.softmax(
            gating_coefficients_unnormalised, dim=1
        )
        ...
        if self.use_best_expert:
            ...
            expert_predictions = torch.split(
                x[1], x[1].shape[1] // self.num_experts, dim=1
            )
            weighted_sum = torch.stack(
                [expert_predictions[j][i] for i, j in enumerate(best_expert_index)]
            )
        else:
            ...
            weighted_sum = sum(
                coeff.unsqueeze(1) * pred
                for coeff, pred in zip(
                    gating_coefficients_normalised.unbind(1), expert_predictions
                )
            )
