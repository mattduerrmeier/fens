...
class SVHNBasicQAgg(Dataset):
    def __init__(self, size, args):
        ...
        if not self.args.proxy_set:
            raise ValueError("Proxy set must be specified for SVHNBasicQAgg")
        if not self.args.logitpath:
            raise ValueError("Logit path must be specified for SVHNBasicQAgg")
    ...
    def load_trainset(self):
        ...
        self.num_samples = np.array(
            [len(self.logit_trainset[i]) for i in range(len(self.logit_trainset))]
        )
    ...
    def load_testset(self):
        ...
        local_logit_testset_path = os.path.join(
            self.args.logitpath, "logit_local_testset.pth"
        )
        ...
        logging.info(
            "==> val set size: {}".format(
                len(self.logit_valset) if self.logit_valset else 0
            )
        )
    ...
    def fetch(self, client_index):
        train_loader = torch.utils.data.DataLoader(
            self.logit_trainset[client_index],
        ...
        logging.info(
            "==> Client id {}, samples from trainset {}, samples from proxyset: {}".format(
                client_index, len(self.logit_trainset[client_index]), 0
            )
        )
