...
class SVHNBasicAgg(Dataset):
    def __init__(self, size, args):
        ...
        if not self.args.proxy_set:
            raise ValueError("Proxy set must be specified for SVHNBasic")
        if not self.args.logitpath:
            raise ValueError("Logit path must be specified for SVHNBasicAgg")
    ...
    def load_trainset(self):
        ...
        if self.args.tr_subset:
            ...
            logit_trainset, _ = random_split(
                logit_trainset, [sub_len, data_len - sub_len], generator=self.generator
            )
