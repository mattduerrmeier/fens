...
class SVHNBasicQAggAdv(Dataset):
    def __init__(self, size, args):
        ...
        if not self.args.proxy_set:
            raise ValueError("Proxy set must be specified for SVHNBasicQAggAdv")
        if not self.args.logitpath:
            raise ValueError("Logit path must be specified for SVHNBasicQAggAdv")
    ...
    def load_trainset(self):
        ...
        for i in range(len(self.logit_trainset)):
            self.logit_trainset[i] = [
                ((x1, x2), y) for x1, x2, y in self.logit_trainset[i]
            ]
