...
class SVHN(Dataset):
    ...
    def load_trainset(self):
        ...
        combined_dataset = ConcatDataset([trainset, extraset])
        ...
        if self.args.tr_subset:
            ...
            combined_dataset, _ = random_split(
                combined_dataset,
                [sub_len, data_len - sub_len],
                generator=self.generator,
            )
        self.trainset = combined_dataset

        logging.info("==> train set size: {}".format(len(self.trainset)))

        partition_sizes = [1.0 / self.size for _ in range(self.size)]
        self.partition = DataPartitioner(
            self.trainset,
            partition_sizes,
            isNonIID=self.args.NIID,
            num_classes=self.num_classes,
            seed=self.args.seed,
            alpha=self.args.alpha,
            dataset=self.args.dataset,
            proxyset=self.args.proxy_set,
            proxy_ratio=self.args.proxy_ratio,
        )
    ...
    def load_testset(self):
        ...
        if self.args.val_set:
            ...
            self.valset, self.testset = random_split(
                testset, [val_len, len(testset) - val_len], generator=self.generator
            )
        else:
    ...
    def fetch(self, client_index):
        ...
        if self.args.proxy_set:
            ...
            proxy_loader = torch.utils.data.DataLoader(
                proxy_data, batch_size=self.args.test_bs, shuffle=True, num_workers=2
            )
            logging.info(
                "==> Client id {}, train dataset size {}, proxy set size: {}".format(
            ...
            logging.info(
                "==> Client id {}, train dataset size {}, proxy set size: {}".format(
                    client_index, len(client_data), 0
                )
            )

        train_loader = torch.utils.data.DataLoader(
            client_data, batch_size=self.args.bs, shuffle=True, pin_memory=True
        )
        test_loader = torch.utils.data.DataLoader(
            self.testset, batch_size=self.args.test_bs, shuffle=False, num_workers=2
        )
        val_loader = None
        if self.valset:
            val_loader = torch.utils.data.DataLoader(
                self.valset, batch_size=self.args.test_bs, shuffle=False, num_workers=2
            )

        local_testset = self.get_local_testset(client_index)
        local_test_loader = torch.utils.data.DataLoader(
            local_testset, batch_size=self.args.test_bs, shuffle=False, num_workers=1
        )
    ...
    def get_proxyset(self):
        if self.args.proxy_set:
            proxy_loader = torch.utils.data.DataLoader(
                self.proxyset, batch_size=self.args.test_bs, shuffle=True, num_workers=2
            )
            return proxy_loader
        else:
    ...
    def get_label_dist(self, client_index):
        if not self.label_dists:
            ...
            for i in range(self.size):
                ...
                local_train_loader = torch.utils.data.DataLoader(
                    client_data,
    ...
    def get_local_testset(self, client_index):
        ...
        if self.local_testsets is None:
            self.local_testsets = distribute_testset(
                self.testset, self.label_dists, self.generator
            )
