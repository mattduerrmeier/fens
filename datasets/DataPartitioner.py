...
class DataPartitioner(object):
    ...
    def __init__(
        ...
        if isNonIID:
            self.partitions, self.ratio = self.__getDirichletData__(
                data, sizes, self.rng, alpha
            )
            total_partitions = len(self.partitions)
            if self.proxyset:
                self.train_partitions = []
                self.proxy_partitions = []
                ...
                for i in range(total_partitions):
                    ...
                    train_partition, proxy_partition = random_split(
                        self.partitions[i],
                    ...
                    self.ratio[i] = (
                        partition_len - proxy_len
                    )  # update to exclude proxy set
    ...
    def use(self, partition):
        if self.proxyset:
            return Partition(self.data, self.train_partitions[partition]), Partition(
                self.data, self.proxy_partitions[partition]
            )
...
def distribute_testset(test_set, label_dists, generator):
    ...
    total_counts = [
        sum(client_dists[c] for client_dists in label_dists) for c in range(num_classes)
    ]
    ...
    for c in range(num_classes):
        ...
        shuffled_indices = torch.randperm(
            len(class_indices_list), generator=generator
        ).tolist()
        ...
        per_client_counts = [
            int(proportions[client_id][c] * class_count)
        ...
        for client_id, num_items in enumerate(per_client_counts):
            ...
            client_partitions[client_id].index.extend(
                class_indices_list[start_index:end_index]
            )
    ...
    logging.debug(
        "Client {} label distribution: {}".format(
            random_client_id, label_dists[random_client_id]
        )
    )
    logging.debug(
        "Client {} test distribution: {}".format(
            random_client_id, test_label_dists[random_client_id]
        )
    )
    logging.debug(
        "Client {} test size: {}".format(
            random_client_id, len(client_partitions[random_client_id])
        )
    )
    total_test_set_size = sum([len(client_partitions[i]) for i in range(num_clients)])
    logging.debug(
        "Total test set size of distributed clients: {}".format(total_test_set_size)
    )
