...
def get_dataset(size, dataset, args):
    datasets = {
        "CIFAR10": CIFAR10,
        "CIFAR10Agg": CIFAR10Agg,
        "CIFAR10QAgg": CIFAR10QAgg,
        "CIFAR10QAggAdv": CIFAR10QAggAdv,
        "SVHN": SVHN,
        "SVHNAgg": SVHNAgg,
        "SVHNBasic": SVHNBasic,
        "SVHNBasicAgg": SVHNBasicAgg,
        "SVHNBasicQAgg": SVHNBasicQAgg,
        "SVHNBasicQAggAdv": SVHNBasicQAggAdv,
        "CIFAR100": CIFAR100,
        "CIFAR100Agg": CIFAR100Agg,
        "CIFAR100QAgg": CIFAR100QAgg,
        "CIFAR100QAggAdv": CIFAR100QAggAdv,
        "FedHeartDiseaseAgg": FedHeartDiseaseAgg,
        "FedISIC2019Agg": FedISIC2019Agg,
        "FedCamelyon16Agg": FedCamelyon16Agg,
    }
...
def select_model(args, rank=0, q=False):
    ...
    if args.model == "VGG":
    ...
    elif args.model == "SmallNN_FHD":
        model = SmallNN_FHD()
    elif args.model == "SmallNN_FCAM":
        model = SmallNN_FCAM()
    elif args.model == "WeightedNN":
        model = WeightedAggregatorNN(
            total_clients=args.totalclients, num_classes=get_num_classes(args.dataset)
        )
        ...
        model = FederatedMoE2(
            num_experts=args.totalclients, num_classes=get_num_classes(args.dataset)
        )
...
def load_models_and_label_dist(my_model, save_dir, map_location=None, use_best=False):
    ...
    if not use_best:
        model_files = [
            f
            for f in all_files
            if f.endswith("_model.pth") and not f.endswith("_best_model.pth")
        ]
    else:
    ...
    for f in model_files:
        if map_location:
            loaded_state = torch.load(
                os.path.join(save_dir, f), map_location=map_location
            )
        else:
    ...
    label_files = [
        f
        for f in all_files
        if f.endswith("_label.pk") or f.endswith("_label_distribution.pk")
    ]
