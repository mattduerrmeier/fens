...
def get_args():
    ...
    # wandb
    parser.add_argument(
        "--wandb_project", default="fens", type=str, help="wandb project name"
    )
    parser.add_argument(
        "--wandb_entity", default="fens", type=str, help="wandb entity name"
    )

    # dataset
    parser.add_argument(
        "--alpha", default=0.2, type=float, help="control the non-iidness of dataset"
    )
    parser.add_argument(
        "--p",
        "-p",
    ...
    parser.add_argument(
        "--NIID", action="store_true", help="whether the dataset is non-iid or not"
    )
    parser.add_argument(
        "--dataset",
        default="CIFAR10",
    ...
    parser.add_argument(
        "--include_trainset",  # for Agg datasets
        action="store_true",
        help="include a fraction of the training set in the proxy set for Agg datasets",
    )
    parser.add_argument(
        "--include_trainset_frac",  # for Agg datasets
        default=0.1,
        type=float,
        help="fraction of training set to be included in the proxy set for Agg datasets",
    )
    parser.add_argument(
        "--tr_subset", action="store_true", help="use a subset of the training set"
    )
    parser.add_argument(
        "--tr_subset_frac", default=0.8, type=float, help="size of the training subset"
    )
    parser.add_argument(
        "--apply_augmentation",
        type=bool,
    ...
    parser.add_argument(
        "--d", default=4, type=int, help="parameter of the small nn aggregator network"
    )
    parser.add_argument(
        "--diff_init",
        action="store_true",
    ...
    parser.add_argument("--slr", default=1.0, type=float, help="server learning rate")
    parser.add_argument(
        "--updatelr", action="store_true", help="Decays learning rate if set"
    )
    parser.add_argument(
        "--gmf", default=0, type=float, help="global (server) momentum factor"
    )
    parser.add_argument(
        "--momentum", default=0.0, type=float, help="local (client) momentum factor"
    )
    parser.add_argument(
        "--bs", default=512, type=int, help="batch size on each worker/client"
    )
    parser.add_argument(
        "--test_bs", default=32, type=int, help="batch size for testing"
    )
    parser.add_argument(
        "--rounds", default=200, type=int, help="total coommunication rounds"
    )
    parser.add_argument(
        "--lowE",
        default=2,
    ...
    parser.add_argument(
        "--optimizer", default="fedavg", type=str, help="optimizer name"
    )
    parser.add_argument(
        "--use_scheduler",
        action="store_true",
    ...
    parser.add_argument(
        "--initmethod", default="tcp://h0:22000", type=str, help="init method"
    )
    ...
    # other
    parser.add_argument(
        "--print_freq", default=100, type=int, help="print info frequency"
    )
    parser.add_argument(
        "--evalafter",
        default=1,
    ...
    parser.add_argument(
        "--save", "-s", action="store_true", help="whether save the training results"
    )
    parser.add_argument(
        "--save_model",
        "-sm",
    ...
    parser.add_argument(
        "--logitpath",  # for Agg datasets
        default=None,
        type=str,
        help="directory from where to load trained model for aggregator training",
    )
    parser.add_argument(
        "--datapath", default="./data/", type=str, help="directory to load data"
    )
    parser.add_argument(
        "--model_init_path",
        default=None,
